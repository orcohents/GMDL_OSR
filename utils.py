...
def find_anchor_means(train_loader, net, device, config, only_correct = False):
    ...
    logits, labels = gather_outputs(net, train_loader, device, only_correct = only_correct)
...
def gather_outputs(net, dataloader, device, data_idx = 0, calculate_scores = False, only_correct = False):
    ...
    for i, data in enumerate(dataloader):
        ...
        if only_correct:
            if data_idx == 0:
                ...
                _, predicted = torch.min(distances, 1) # predicted is the class of the prediction 
    ...
    # Get the maximum values and indices along axis=1
